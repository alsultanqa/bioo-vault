...
<body>
  ...
  <script>
// (اختياري) لفتح لوحة auth اليدوية بدون Hosted UI
window.mbShowAuthPanel = function () {
...
const COGNITO_DOMAIN = 'minibankpwa-coobio.auth.eu-north-1.amazoncognito.com';
const COGNITO_CLIENT = '6b482qijvo92ofpc4uqdd76f1u';
const REDIRECT_URI   = 'https://alsultanqa.github.io/bioo-vault/';
...
<script>
  (function () {
    ...
    async function boot() {
      try {
        // 1) حمّل Ethers (للوالدجيت البلوكتشين)
        await loadEthers();

        // 2) (اختياري) لو تحتاج SDK إضافي، حمّله هنا داخل boot وليس خارجها
        // مثال: await inject('https://cdn.jsdelivr.net/npm/amazon-cognito-identity-js@6.3.9/dist/amazon-cognito-identity.min.js');

      } catch (e) {
        console.error('[BioVault] Failed to load ethers from both CDNs — blockchain features may be disabled.', e);
      } finally {
        try {
          // 3) أخيرًا: حمّل تطبيقك
